...
pub fn get_datatypes() -> Vec<PartialNamespace> {
    ...
    let udim2 = datatype!(UDim2);
    let udim = datatype!(UDim);
    ...
    namespaces! {
        ...
        struct UDim: DataType {
            static new(scale: number, offset: number) -> (udim);

            readonly Scale: number;
            readonly Offset: number;
        };

        struct UDim2: DataType {
            static new::new() -> (udim2);
            static new::new_with_components(x_scale: number, x_offset: number, y_scale: number, y_offset: number) -> (udim2);
            static new::new_with_udims(x: udim, y: udim) -> (udim2);
            static fromScale(x: number, y: number) -> (udim2);
            static fromOffset(x_offset: number, y_offset: number) -> (udim2);
